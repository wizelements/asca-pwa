    ...
    <html>
      <body style="font-family: system-ui, -apple-system, sans-serif; color: #1f1f1f; background: #f7f3ea; padding: 24px;">
        <div style="max-width: 560px; margin: 0 auto; background: #ffffff; border-radius: 12px; padding: 24px; border: 1px solid #e3dac9;">
          ...
          <p>This is a friendly reminder that your quarterly dues are due on <strong>${escapeHtml(data.dueDate)}</strong> (${data.daysRemaining} days from today).</p>
    ...
    <html>
      <body style="font-family: system-ui, -apple-system, sans-serif; color: #1f1f1f; background: #f7f3ea; padding: 24px;">
        <div style="max-width: 560px; margin: 0 auto; background: #ffffff; border-radius: 12px; padding: 24px; border: 1px solid #e3dac9;">
          ...
          <p>Hi ${escapeHtml(data.name)},</p>
          <p>Your quarterly dues were due on <strong>${escapeHtml(data.dueDate)}</strong>. Please submit your dues as soon as possible.</p>
      ...
      <body style="font-family: system-ui, -apple-system, sans-serif; color: #333;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr>
            ...
            <td style="padding: 8px; border-bottom: 1px solid #eee;">${escapeHtml(data.role)}</td>
          </tr>
          <tr>
            <td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Experience:</strong></td>
            <td style="padding: 8px; border-bottom: 1px solid #eee;">${escapeHtml(data.experience)}</td>
          </tr>
        </table>
        <hr />
        <h3>Message:</h3>
    ...
    <html>
      <body style="font-family: system-ui, -apple-system, sans-serif; color: #333;">
        ...
        <div style="background: #f9f9f9; padding: 16px; border-left: 4px solid #4a4b02;">
          <h3 style="color: #1a1a1a; margin-top: 0;">${escapeHtml(data.eventTitle)}</h3>
          ...
          <p><strong>Status:</strong> <span style="color: #4a4b02; font-weight: bold;">${escapeHtml(data.rsvpStatus)}</span></p>
        </div>
        <p>See you there!</p>
        <p style="color: #666;">
  ...
  return `
    <html>
      <body style="font-family: system-ui, -apple-system, sans-serif; color: #333;">
        ...
        <p>Hi ${escapeHtml(data.donorName)},</p>
        ...
        <p style="background: #f9f9f9; padding: 12px; border-left: 4px solid #4a4b02;">
          <strong>Transaction ID:</strong> ${escapeHtml(data.transactionId)}
        </p>
  ...
  return `
    <html>
      <body style="font-family: system-ui, -apple-system, sans-serif; color: #333;">
        ...
        <p>Hi ${escapeHtml(data.volunteerName)},</p>
        <p>Thank you for signing up to volunteer with the Atlanta Saddle Club Association!</p>
        <div style="background: #f9f9f9; padding: 16px; border-left: 4px solid #4a4b02;">
          <h3 style="color: #1a1a1a; margin-top: 0;">${escapeHtml(data.opportunityTitle)}</h3>
          <p>We'll be in touch soon with more details.</p>
          <p><strong>Questions?</strong> Contact us at <a href="mailto:${escapeHtml(data.contactEmail)}">${escapeHtml(data.contactEmail)}</a></p>
        </div>
        <p style="color: #666;">
          <em>Atlanta Saddle Club Association</em><br />
          We Ride To Inspire
    ...
    <html>
      <body style="font-family: system-ui, -apple-system, sans-serif; color: #333; background: #f7f3ea; padding: 24px;">
        <div style="max-width: 560px; margin: 0 auto; background: #ffffff; border-radius: 12px; padding: 24px; border: 1px solid #e3dac9;">
          ...
          <p style="background: #f9f9f9; padding: 12px; border-left: 4px solid #1f6b3a;"><strong>${escapeHtml(data.adminEmail)}</strong></p>
          ...
          <p style="margin: 24px 0;">
            <a href="${escapeHtml(data.resetUrl)}" style="display: inline-block; background: #1f6b3a; color: #ffffff; padding: 12px 18px; border-radius: 8px; text-decoration: none; font-weight: 700;">Reset Admin Password</a>
          </p>
          <p style="font-size: 12px; color: #666; word-break: break-all;">If the button does not work, copy this link:<br />${escapeHtml(data.resetUrl)}</p>
